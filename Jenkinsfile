...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-shipping-service', url: 'https://github.com/sylvanus2023/realworld-microservice-project.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t sylvanus2022/shippingservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker sylvanus2022/shippingservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push sylvanus2022/shippingservice:latest "
                    }
                }
            }
        }
